...
describe NNTP::Pool do
  it "should use proc to create objects" do
    block_called = 0
    pool = NNTP::Pool.new(initial_pool_size: 3) { block_called += 1; Closable.new }
    block_called.should eq(3)
  end

  it "should get resource" do
    pool = NNTP::Pool.new { Closable.new }
  ...
  it "should be available if not checkedout" do
    ...
    pool = NNTP::Pool.new(initial_pool_size: 1) { resource = Closable.new }
  ...
  it "should wait for available resource" do
    pool = NNTP::Pool.new(max_pool_size: 1, initial_pool_size: 1) { Closable.new }
  ...
  it "should create new if max was not reached" do
    block_called = 0
    pool = NNTP::Pool.new(max_pool_size: 2, initial_pool_size: 1) { block_called += 1; Closable.new }
  ...
  it "should reuse returned resources" do
    all = [] of Closable
    pool = NNTP::Pool.new(max_pool_size: 2, initial_pool_size: 1) { Closable.new.tap { |c| all << c } }
    pool.checkout
    b1 = pool.checkout
    pool.release b1
    b2 = pool.checkout
  ...
  it "should timeout" do
    pool = NNTP::Pool.new(max_pool_size: 1, checkout_timeout: 0.1) { Closable.new }
    pool.checkout
    expect_raises NNTP::Error::PoolTimeout do
      pool.checkout
    end
  end
  ...
  it "should close if max idle amount is reached" do
    all = [] of Closable
    pool = NNTP::Pool.new(max_pool_size: 3, max_idle_pool_size: 1) { Closable.new.tap { |c| all << c } }
    pool.checkout
    pool.checkout
    pool.checkout

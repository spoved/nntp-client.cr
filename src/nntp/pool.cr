class NNTP::Pool(T)
  ...
  # maximum amount of retry attempts to reconnect to the client. See `Pool#retry`
  ...
  def retry
    ...
    (current_available + @retry_attempts).times do |i|
      begin
        ...
        Log.error(exception: e) { "[#{Fiber.current.name}] nntp.pool.retry resource refused. attempts: #{@retry_attempts} current: #{i}. msg: #{e.message}" }
      end
    end

    raise NNTP::Error::PoolRetryAttemptsExceeded.new
  end
  ...
  {% if compare_versions(Crystal::VERSION, "0.34.0-0") > 0 %}
    private def wait_for_available
      ...
      when timeout(@checkout_timeout.seconds)
        sync_dec_waiting_resource
        raise NNTP::Error::PoolTimeout.new("Could not check out a connection in #{@checkout_timeout} seconds")
      end
    end
  {% else %}
    private def wait_for_available
